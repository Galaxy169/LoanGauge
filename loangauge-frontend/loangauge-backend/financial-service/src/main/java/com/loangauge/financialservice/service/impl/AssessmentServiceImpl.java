...
public class AssessmentServiceImpl implements AssessmentService {
    ...
    @Override
    @Transactional
    public AssessmentResponseDto createAssessment(Long userId, String role, String userEmail, AssessmentRequestDto request) {
...
//        }
        ...
        publishAssessmentCompleted(saved, userId, userEmail, role, profile);
    ...
    private void publishAssessmentCompleted(LoanAssessment saved, Long userId, String userEmail, String userRole, ProfileResponseDto profile) {
        try {
            Map<String, Object> event = new java.util.HashMap<>();

            // --- Identity ---
            event.put("assessmentId", saved.getAssessmentId());
            event.put("userId", userId);
            event.put("userEmail", userEmail);
            event.put("userRole", userRole);

            // --- Loan details ---
            event.put("loanType",       saved.getLoanType().getLoanName());
            event.put("loanAmount",     saved.getLoanAmount());
            event.put("tenureMonths",   saved.getTenureMonths());
            event.put("interestRate",   saved.getInterestRate());
            event.put("proposedEmi",    saved.getEmi());

            // --- Computed metrics ---
            event.put("foir",                       saved.getFoir());
            event.put("dti",                        saved.getDti());
            event.put("savingsRatio",               saved.getSavingsRatio());
            event.put("emergencyFundCoverageMonths",saved.getEmergencyFundCoverageMonths());
            event.put("creditUtilization",          saved.getCreditUtilization());
            event.put("disposableIncome",           saved.getDisposableIncome());
            event.put("financialReadinessScore",    saved.getFinancialScore());
            event.put("riskCategory",               saved.getRiskLevel().name());
            event.put("eligibleAmount",             saved.getEligibleAmount());

            // --- Raw profile inputs ---
            event.put("monthlyIncome",      profile.monthlyIncome());
            event.put("monthlyExpenses",    profile.monthlyExpenses());
            event.put("existingEmi",        profile.monthlyEmi());
            event.put("savings",            profile.savings());
            event.put("fixedDeposits",      profile.fixedDeposits());
            event.put("investments",        profile.investments());
            event.put("emergencyFund",      profile.emergencyFund());
            event.put("cibilScore",         profile.cibilScore());
            event.put("existingLoans",      profile.existingLoans());
            event.put("employmentType",     profile.employmentType());
            event.put("incomeStability",    profile.incomeStability());
            event.put("workExperienceYears",profile.workExperienceYears());
            event.put("age",                profile.age());
            event.put("dependents",         profile.dependents());
            event.put("maritalStatus",      profile.maritalStatus());
