...
public class FinancialScoreCalculator {
    ...
    /**
     * Calculates net monthly income remaining after accounting for primary loan
     * EMIs and debt obligations.
     */

    public static BigDecimal calculateDisposableIncome(BigDecimal monthlyIncome, BigDecimal monthlyEmi,
            BigDecimal otherObligations) {
        return nz(monthlyIncome).subtract(nz(monthlyEmi)).subtract(nz(otherObligations))
                .setScale(MONEY_SCALE, ROUNDING);
    }

    // income scores 0; a disposable-income-to-income ratio of 40% or more scores
    // 100,
    ...
    public static int scoreCreditUtilization(BigDecimal creditUtilization) {
        if (creditUtilization.compareTo(BigDecimal.valueOf(30)) < 0)
            return 100;
        if (creditUtilization.compareTo(BigDecimal.valueOf(50)) < 0)
            return 75;
        if (creditUtilization.compareTo(BigDecimal.valueOf(75)) < 0)
            return 50;
        return 25;
    }

    // Credit Profile score is CIBIL-driven using standard Indian CIBIL
    // bands (750+ excellent ... <600 very poor), with a small penalty for more than
    // 2
    ...
    public static int scoreCreditProfile(Integer cibilScore, Integer existingLoanCount) {
        int base;
        if (cibilScore == null) {
            base = 50; // Neutral default when CIBIL is unpopulated Applicant has no credit history
                       // (NH).
            // Treat as neutral instead of poor.
    ...
    // work experience contributes up to 40 points, self-reported income stability
    // up to 60.
    /**
     * Evaluates stability using work experience (max 40 pts) and self-reported
     * income stability (max 60 pts).
     */
    ...
    /**
     * Combines all 8 weighted sub-scores (each pre-normalized 0-100) into the final
     * score.
     */
    ...
    public static RiskLevel classifyRisk(int financialScore) {
        ...
        if (financialScore >= 40)
            return RiskLevel.NEEDS_IMPROVEMENT;
        return RiskLevel.HIGH_RISK;
    }

    /**
     * Affordability guardrail: no matter how healthy the other metrics are, a loan
     * the
     * user fundamentally cannot afford (obligations far exceeding income, or
     * negative
     * disposable income) must not score as merely "moderate". A weighted average
     * alone
    ...
    public static int applyAffordabilityCap(int weightedScore, BigDecimal foir, BigDecimal disposableIncome) {
        // Negative disposable income = cannot service the loan at all → hard cap at
        // High Risk band.
        if (disposableIncome != null && disposableIncome.compareTo(BigDecimal.ZERO) < 0) {
            return Math.min(weightedScore, 39); // forces HIGH_RISK
        }
        // FOIR beyond ~75% is unaffordable by any lender's standard → cap at Needs
        // Improvement.
